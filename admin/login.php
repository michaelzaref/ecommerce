...
if(isset($_POST['email']) and isset($_POST['password'])){
  ...
  $first=1;
  $get_cat_i = "SELECT * FROM `admin` WHERE `email`='".$_POST['email']."';"; 
  ...
  if($arr!= NULL){
    ...
    if($password){
      ...
      if($password==$_POST['password']){
        ...
        $se_id = "UPDATE `admin` SET `session`='".$_SESSION['id']."' WHERE `name`='".$_SESSION['name']."'";
        mysqli_query($conn, $se_id);
  
  
  
        $get_img = "SELECT * FROM `admin` WHERE `email`='".$_POST['email']."';";
